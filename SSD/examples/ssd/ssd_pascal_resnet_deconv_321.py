...
def AddDeconvLayers(net, forward_layers, use_batchnorm=True):
    ...
    name = CombineFeature(net, 'combined_8','deconv_8', forward_layers[0], method=deconv_combine_method)
    output_layers.append(name)

    #5x5
    DeconvBNLayer(net, name, 'deconv_7', False, False, 512, 3, 0, 1)
    name = CombineFeature(net, 'combined_7','deconv_7', forward_layers[1], method=deconv_combine_method)
    output_layers.append(name)

    #10x10
    DeconvBNLayer(net, name, 'deconv_6', False, False, 512, 2, 0, 2)
    name = CombineFeature(net, 'combined_6','deconv_6', forward_layers[2], method=deconv_combine_method)
    output_layers.append(name)

    #20x20
    DeconvBNLayer(net, name, 'deconv_5', False, False, 512, 2, 0, 2)
    name = CombineFeature(net, 'combined_5','deconv_5', forward_layers[3], method=deconv_combine_method)
    output_layers.append(name)

    # 40x40
    DeconvBNLayer(net, name, 'deconv_3', False, False, 512,  2, 0, 2)
    name = CombineFeature(net, 'combined_3','deconv_3', forward_layers[4], method=deconv_combine_method)
    output_layers.append(name)
...
job_name = "DSSD_VOC07_{}".format(resize)
...
output_result_dir = "./results/VOC2012/{}/Main".format(job_name)
...
name_size_file = "data/VOC0712/VOC2007_test_name_size.txt"
...
ResNet101Body(net, from_layer='data', use_pool5=False, use_dilation_conv5=True,
            lr_mult=0, decay_mult=0, **bn_param)

# Use batch norm for the newly added layers.
AddExtraLayers(net, use_batchnorm=True, lr_mult=0, decay_mult=0, **bn_param)

deconv_layers = AddDeconvLayers(net, mbox_source_layers[4::-1], use_batchnorm=True)
